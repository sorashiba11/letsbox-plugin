...
`codex mcp logout letsbox` だけではブラウザ側のLetsBoxログインが残るため、再ログインすると**同じアカウントで自動的に認証されてしまう**。切り替えには、先にブラウザ側のログアウト（手順2）が必須。

## 手順

1. `get_connection_status` を呼び、「現在は ○○@…（会社名）のアカウントです」と伝えたうえで、切り替えでよいか確認する。
2. 利用者に次の2ステップを案内し、完了の返事を待つ:
   - 「①ブラウザで https://auth.letsai.team/logout を開いてください。『ログアウトしました』と表示されたらOKです」
...
4. 完了の返事が来たら `get_connection_status` を呼び直し、`account_email` が切り替わったことを確認して「○○@… に切り替わりました」と報告する。
   - 切り替わっていない場合は、手順2の①（ブラウザ側ログアウト）が抜けている可能性が高い。責めずに①からやり直しを案内する。
